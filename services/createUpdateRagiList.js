...
const ragiListUrl = process.env.ragiListUrl;
  ...
  return extractTables(getRagiListBuffer).then(tables => {
    tables = tables.map((arr) => {
      if (arr[0].text.includes('Bhai'))
        return [{
          text:
            arr[0].text.split('Bhai').map((data) => data.split('Singh').map((data) => data.replace(/\s+/g, '')).join(" Singh ")).join(' Bhai ') //clearing empty spaces in ragi name data as conversion has some unnecessary spaces
        }]
      return arr
    })
...
const generateTimingsArrayFromString = (str) => {
  const regexToDetectTimeInFormat_HH_MM = /(\d{1,2}-\d{2}).*?to.*?((\d{1,2}-\d{2}(?:.*?Morning)?)|till Completion)/gm;
  const timingsArray = [];
  let match;
